...
const STORAGE_KEY = "projects-page-notice";

const defaultNotice: NoticeData = {
  title: "项目公告",
  content:
    "可在此填写项目页面说明，例如协作规范、近期重点、资源维护注意事项等，帮助团队快速对齐上下文。",
  publisher: "项目管理员",
...
export function ProjectsNotice() {
  ...
  const handleClear = () => {
    if (!window.confirm("确定要清空项目公告吗？此操作无法撤销。")) {
      return;
    }
  ...
  return (
    <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-slate-900">页面公告 / 项目说明</h3>
        {!editing ? (
          <button
            onClick={handleEdit}
      ...
      {!editing ? (
        ...
        <div className="mt-3 space-y-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
          <div>
            <label
              htmlFor="projects-notice-title"
              className="mb-1 block text-xs text-slate-500"
            >
              公告标题
            </label>
            <input
              id="projects-notice-title"
              value={draft.title}
              onChange={(event) =>
          ...
          <div>
            <label
              htmlFor="projects-notice-content"
              className="mb-1 block text-xs text-slate-500"
            >
              公告内容
            </label>
            <textarea
              id="projects-notice-content"
              value={draft.content}
              onChange={(event) =>
          ...
          <div>
            <label
              htmlFor="projects-notice-publisher"
              className="mb-1 block text-xs text-slate-500"
            >
              发布人
            </label>
            <input
              id="projects-notice-publisher"
          ...
          <label
            htmlFor="projects-notice-pinned"
            className="flex items-center gap-2 text-sm text-slate-700"
          >
            <input
              id="projects-notice-pinned"
              type="checkbox"
